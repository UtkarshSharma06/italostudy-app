...
// We need to completely replace the <motion.div key="pricing-modal-desktop" ...> ... </motion.div>
// To do this reliably, we'll find the start and the next </motion.div> that corresponds to it.

const desktopStart = '<motion.div\r\n                            key="pricing-modal-desktop"';
let startIdx = content.indexOf(desktopStart);
if (startIdx === -1) {
    startIdx = content.indexOf('<motion.div\n                            key="pricing-modal-desktop"');
}

if (startIdx !== -1) {
    // Find the end of this div. We can just use the fact that it's the last motion.div before </AnimatePresence>
    const animatePresenceEnd = content.indexOf('</AnimatePresence>', startIdx);
    // The </motion.div> for the desktop UI is right before </AnimatePresence>
    let endIdx = content.lastIndexOf('</motion.div>', animatePresenceEnd);
    endIdx += '</motion.div>'.length;
    ...
    const newContent = content.substring(0, startIdx) + newDesktopUI + content.substring(endIdx);
    fs.writeFileSync(targetPath, newContent, 'utf8');
    console.log('Successfully refactored desktop UI layout');
} else {
    console.error("Could not find start of desktop UI");
}
