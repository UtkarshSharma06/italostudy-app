...
import { Calendar, Clock, Users, Globe, Play, ChevronRight, Zap, Target, ShieldCheck, Loader2, Crown, Lock, Sparkles, RotateCcw, Layers, X, FileDown } from 'lucide-react';
                                            ...
                                            return (
                                                <div key={session.id} className="bg-white dark:bg-card p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border-2 border-slate-100 dark:border-border border-b-[6px] shadow-xl shadow-slate-200/50 hover:border-slate-300 hover:-translate-y-1 hover:shadow-2xl active:border-b-2 active:translate-y-1 transition-all duration-300 group relative overflow-hidden" onClick={() => isLocked && setIsUpgradeModalOpen(true)}>
                                                    ...
                                                    <div className="flex flex-col gap-2 w-full">
                                                        ...
                                                        {session.config?.pdf_url && (
                                                            <Button onClick={(e) => { e.stopPropagation(); if (!hasPremiumAccess) { setIsUpgradeModalOpen(true); return; } window.open(session.config.pdf_url, '_blank'); }} className="w-full h-12 sm:h-14 font-black rounded-2xl text-[9px] sm:text-[10px] uppercase tracking-[0.15em] transition-all shadow-sm bg-purple-50 dark:bg-purple-500/10 text-purple-600 dark:text-purple-400 border border-purple-200 dark:border-purple-500/30 hover:bg-purple-100 dark:hover:bg-purple-500/20 flex items-center justify-center gap-2">
                                                                {hasPremiumAccess ? <FileDown className="w-4 h-4" /> : <Lock className="w-4 h-4" />}
