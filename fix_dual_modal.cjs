...
// We need to find the specific closing sequence for the mobile modal:
/*
                                </p>
                            </div>
                        </motion.div>
...
*/
const searchPattern = '                        </motion.div>\r\n                    </motion.div>\r\n                )}\r\n            </AnimatePresence>';
let insertIdx = content.indexOf(searchPattern);

if (insertIdx === -1) {
    // Try without \r
    const searchPattern2 = '                        </motion.div>\n                    </motion.div>\n                )}\n            </AnimatePresence>';
    insertIdx = content.indexOf(searchPattern2);
}

if (insertIdx !== -1) {
    // We want to insert AFTER the FIRST </motion.div> in that sequence.
    // That means: insertIdx + 37 (length of "                        </motion.div>")
    // Let's just find the first </motion.div> in the matched block and insert right after it.
    const firstEndDiv = content.indexOf('</motion.div>', insertIdx);
    const splitPoint = firstEndDiv + '</motion.div>'.length;
    
    const insertStr = content.substring(0, splitPoint) 
                    + '\n' + desktopUI + '\n' 
                    + content.substring(splitPoint);
                    
    fs.writeFileSync(targetPath, insertStr, 'utf8');
    console.log('Fixed dual modal rendering precisely inside the overlay.');
} else {
    console.error("Could not find exact injection sequence.");
}
